...
class addsnow(object):
    def __init__(self, snow_coeff=-1):
        self.snow_coeff = snow_coeff
        print("selected snow_coeff value - ",self.snow_coeff)

    def __call__(self, img, bboxes):
        self.snow_coeff =random.uniform(0,1)
        self.snow_coeff *=255/2
        self.snow_coeff +=255/3

        image_HLS = cv2.cvtColor(img,cv2.COLOR_RGB2HLS) ## Conversion to HLS
        image_HLS = np.array(image_HLS, dtype = np.float64) 
        brightness_coefficient = 2.5 
        imshape = img.shape
        snow_point=self.snow_coeff  ## increase this for more snow
...
class RandomBrightness(object):
    def __init__(self, gamma=1.2):
        self.gamma = gamma
        print("selected gamma value - ",self.gamma)

    def __call__(self, img, bboxes):
        if self.gamma == 1:
            print("gamma value is 1 which represent no change required...!!!")
...
class GaussianFiltering(object):
    def __init__(self, filter_size=5):
        ...
        print("selected kernel filter - ",filter_size)

    def __call__(self, img, bboxes):
        blur = cv2.GaussianBlur(img,self.filter_size,0)  
        return blur, bboxes
...
class RandomRotate(object):
    ...
    def __call__(self, img, bboxes):
        ...
        #print("corners in randome -",corners)
...
#                print("bboxes in sequence -",type(bboxes))
                if bboxes.size == 0:
                    #print("bboxes is empty ",bboxes)
                    continue
                else:
