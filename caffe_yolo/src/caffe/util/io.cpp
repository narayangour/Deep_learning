...
int IGNORE_FLAG(0); //added flag to ignore some specif class
int object_cnt(0); 
...
bool ReadBoxDataToDatum(const string& filename, const string& annoname,
  ...
  if (cv_img.data) {
    if (encoding.size()) {
      ...
      datum->set_encoded(true);
      // read xml anno data
      //LOG(INFO)<<"Inside ReadBoxDataToDatum in io.cpp file"<<  annoname;
    ...
    CVMatToDatum(cv_img, datum);
    // read xml anno data
    LOG(INFO)<<"read xml anno data other options";
    ParseXmlToDatum(annoname, label_map, ori_w, ori_h, datum);
    return true;
  } else {
    return false;
...
void ParseXmlToDatum(const string& annoname, const map<string, int>& label_map,
  ...
  //LOG(INFO)<<"annoname inside ParseXmlToDatum  ->"<<annoname;
    ...
    if (v1.first == "object") {
      ...
      BOOST_FOREACH(ptree::value_type &v2, object.get_child("")) {
        ...
        if (v2.first == "name") {
          ...
          //LOG(INFO) << " name: " << name;
          if(((name=="dog")||(name=="horse")||(name=="bird"))&&((bird_cnt<=3000)&&(dog_cnt<=3000))){
          if (name=="bird"){
            if (bird_cnt<3000){
            bird_cnt= bird_cnt+1;
            }else{continue;
            ...
            horse_cnt= horse_cnt+1;   
          }
          //LOG(INFO) << "INSIDE IF condition";
          //object_cnt = object_cnt + 1;
          LOG(INFO) << "name: " << name;
          LOG(INFO) <<"bird_cnt:"<<bird_cnt;
          LOG(INFO) <<"dog_cnt:"<<dog_cnt;
          LOG(INFO) <<"horse_cnt:"<<horse_cnt;
          //LOG(INFO) << "object : "<< object_cnt;
          // map name to label
        ...
        } else if ((v2.first == "difficult")&&(IGNORE_FLAG== 1)) {
          //LOG(INFO) << "seconf else  IF condition";
          //LOG(INFO)<<"Inside else if2 ->"<<IGNORE_FLAG;
          difficult = atoi(pt2.data().c_str());
        }
      }
      if(label>=0){
      //LOG(INFO) << "on final check";
