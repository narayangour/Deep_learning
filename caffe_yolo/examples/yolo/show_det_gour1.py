...
def show_boxes(im_path, boxes, thresh=0.5, show=0):
  ...
  for box in boxes:
    ...
    cv2.putText(im,is_class,(int (x2-100),int(y2-100)),2,1.5,(0,0,255),)
    cv2.rectangle(im, (x1, y1), (x2, y2), (0, 0, 255), 2)
    labelintxt.append(is_class+' ' +str(x1)+ ' '+str(y1)+ ' '+str(x2)+ ' '+str(y2)+ ' '+temp)
    #print ("labelintxt ->",labelintxt)
  if show:
    cv2.imshow("out", im)
    cv2.waitKey(0)
  else:
    ...
    if len(labelintxt):
      #print("image name -> ",out_label+im_path.split('/')[-1])
      file1 = open(out_label+im_path.split('/')[-1].split('.')[0]+'.txt',"w")
    #print ("labelintxt ->",labelintxt,len(labelintxt))
      file1.write(labelintxt[0]) 
...
if __name__=="__main__":
  ...
  model_path = "./test2/gnet_yolo_iter_115500.caffemodel"
  model = caffe.Net(net_proto, model_path, caffe.TEST)


  for idx, testImg in enumerate(test_img_list):
    print "\n",idx,") Input Img path -",testImg
    det(model, testImg, show=1)
  print "Inferencing is Done..!!!"
